                            ...
                            <div class="card-body">
                                <form action="{{ route('group.store') }}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="border p-4 rounded">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <label for="inputEnterYourName" class="col-form-label">Group Name <span style="color: red;">*</span></label>
                                            ...
                                            <div class="col-md-6">
                                                <label for="inputEnterYourName" class="col-form-label">Admin <span style="color: red;">*</span></label>
                                                <select name="admin_id" id="" class="form-control">
                                                    <option value="">Select a Admin</option>
                                                    @foreach ($admins as $admin)
                                                        <option value="{{ $admin->data()['uid'] }}">{{ $admin->data()['name'] }}</option>
                                                    @endforeach
                                                </select>
                                                @if($errors->has('admin_id'))
                                                <div class="error" style="color:red;">{{ $errors->first('admin_id') }}</div>
                                                @endif
                                            </div>
                                            
                                            <div class="col-md-6">
                                                <label for="inputPhoneNo2" class="col-form-label">Members<span style="color:red">*<span></label>
                                                    <select name="user_id[]" id="" class="form-control multi-select" multiple>
                                                        @foreach ($users as $user)
                                                            <option value="{{ $user->data()['uid'] }}">{{ $user->data()['name'] }}</option>
                                                        @endforeach
                                                    </select>
